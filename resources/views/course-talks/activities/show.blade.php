...
@section('content')
    ...
    <x-table title="Ediciones" data-testid="course-talks-activity-editions-table">
        @slot('filters')
            @can('create', App\Models\Courses\CourseEdition::class)
                <a class="btn btn-sm btn-primary" data-testid="course-talks-edition-create-link" href="{{ route('course-talks.editions.create', $activity) }}">Nueva edición</a>
            @endcan
        @endslot
        ...
        @slot('rows')
            ...
            @empty
                <tr><td colspan="5" class="text-center text-secondary py-4">No hay ediciones registradas.</td></tr>
            @endforelse
        @endslot
    </x-table>
@endsection
